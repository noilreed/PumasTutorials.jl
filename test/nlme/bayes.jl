...
@testset "Model with no priors" begin
  ...
  @testset "Allow Diagonal in parameter initialization (issue #1180)" begin
    par = (param..., Ω = Diagonal([1.0]))
    ft = fit(theopmodel_bayes, theopp, par, Pumas.FOCEI())
    @test loglikelihood(ft) ≈ -219.69776784013186 atol=1e-5 
  end
end
