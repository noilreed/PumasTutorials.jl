...
subject = Subject(events = DosageRegimen([10, 20], ii = 24, addl = 2, ss = 1:2, time = [0, 12], cmt = 2))
...
@test [1000 * sol(12*i)[2] / col(0).Vc for i in 0:1] ≈ [605.3220736386598;1616.4036675452326] atol=1e-8
###############################
# Test 16
###############################

subject = Subject(events = DosageRegimen([10, 20, 10], ii = 24, ss = [1,2,1], time = 0:12:24, cmt = 2))
col = pre(m_diffeq, subject, param, randeffs)
sol = solve(m_diffeq, subject, param, randeffs; saveat = 0.0:12.0:60.0, abstol=1e-14,reltol=1e-14)

@test [1000 * sol(12*i)[2] / col(0).Vc for i in 0:5] ≈ [605.3220736386598
                                                        1616.4036675452326
                                                        605.3220736387212
                                                        405.75952026789673
                                                        271.98874030537564
