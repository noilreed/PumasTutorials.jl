...
tmdd = @model begin
    ...
    @vars begin
        ...
        delta = (Ltot-R0-KD)^2 + 4*KD*Ltot
        deltasat = max(delta,0) # saturation to avoid taking square root of negative number (delta might be slightly negative due to small numerical errors)
        L = 1/2*((Ltot-R0-KD) + sqrt(deltasat))

        Rtot = R0                 # total receptor concentration (constant)
        R    = Rtot - (Ltot - L)  # free receptor concentration
        P    = Ltot - L           # complex concentration
